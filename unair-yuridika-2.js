...
async function getDataJurnal(){
	...
	await page.goto("https://e-journal.unair.ac.id/YDK/issue/archive/2", {timeout: 0})
	...
	const nama_jurnal = "Yuridika"//await page.$eval(".data strong", text => text.innerText)
	...
	for(let i = 0; i < linkIssue.length; i++){
		...
		for(let j = 0; j < linkData.length; j++){
			...
			let nama_file
			try{
				linkPDF = await page.$eval(".obj_galley_link", href => href.href)
			}catch{
				linkpdf = "-";
				nama_file = "-"
			}
			...
			if(linkpdf !== "-"){
				...
				nama_file = `${author_file} - ${title_file}.pdf`
			...
			con.connect(function(err) {
			  ...
			  con.query(sql, function(err, result){
			    ...
			    jumlahMasukDB++;
			    console.log(`berhasil menambahkan ${jumlahMasukDB} row`)
			  })
